...
public class islemEksi30 : MonoBehaviour
{
    #region Tanýtýmlar
    ...
    void AriDectivateEksiAlti()
    {
        for (int i = 0; i < 30; i++)
        {
            AriSpawner.Instance.AriParent.transform.GetChild(i+1).gameObject.SetActive(false);//Dost arýyý kapatýyor
            //dusmanParent.transform.GetChild(i).gameObject.SetActive(false);//Duþman arýyý kapatýyor
        }
        AriSpawner.mevcutAriSayi += -30;//toplam arý sayýsýndan 6 çýkarýyor
    }
    #endregion
}
