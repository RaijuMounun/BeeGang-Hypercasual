...
public class islemEksi2 : MonoBehaviour
{
    #region Tanýtýmlar
    ...
    void AriDectivateEksiAlti()
    {
        for (int i = 0; i < 2; i++)
        {
            AriSpawner.Instance.AriParent.transform.GetChild(7-i).gameObject.SetActive(false);//Dost arýyý kapatýyor
            dusmanParent.transform.GetChild(i).gameObject.SetActive(false);//Duþman arýyý kapatýyor
        }
        AriSpawner.mevcutAriSayi += -2;//toplam arý sayýsýndan 6 çýkarýyor
    }
    #endregion
}
