...
public class islemEksi8 : MonoBehaviour
{
    #region Tanýtýmlar
    ...
    void AriDectivateEksiAlti()
    {
        for (int i = 0; i < 8; i++)
        {
            AriSpawner.Instance.AriParent.transform.GetChild(i+32).gameObject.SetActive(false);//Dost arýyý kapatýyor
            dusmanParent.transform.GetChild(i).gameObject.SetActive(false);//Duþman arýyý kapatýyor
        }
        AriSpawner.mevcutAriSayi += -8;//toplam arý sayýsýndan 6 çýkarýyor
    }
    #endregion
}
